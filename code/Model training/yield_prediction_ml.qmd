---
title: "Corn Yield Prediction: XGBoost vs LightGBM"
subtitle: "CRSS 8030 · Final Project Group 8 · 2026"
author: "Group 8"
date: today
format:
  html:
...
| Choice | Rationale |
|---|---|
| Impute numeric NAs with median | Soil variables (pH, OM, K, P) have substantial missingness; median is robust to skew |
| Impute nominal NAs with mode | `previous_crop` and similar factors |
| `step_novel` + `step_other` | Handle new factor levels in test set; lump rare hybrids (<0.5%) to "other" |
| `step_dummy` (one-hot) | Required by XGBoost; LightGBM handles this natively but one-hot works for both |
| `step_nzv` | Remove near-zero-variance dummies created from lumping |
...
**Choice:** 5-fold cross-validation repeated 2 times, stratified on `yield_mg_ha`
(cut into quartiles for stratification).

- Large training set (~107 k rows) means 5 folds give ample validation signal.
- Repetition (×2) reduces variance in CV estimates without excessive compute cost.
- Stratification ensures each fold has a balanced yield distribution.
...
LightGBM was chosen as the second model because:

- Leaf-wise tree growth often achieves lower RMSE than XGBoost's level-wise growth on tabular data with many categorical columns.
- Typically 3–5× faster to train, enabling more thorough hyperparameter search.
- Native support for high-cardinality categoricals (hybrids, sites) before one-hot encoding.
...
**Search strategy:** Latin Hypercube Space-Filling Design (size = 40 per model)
followed by **Racing via ANOVA** (`tune_race_anova`).

- LHD covers the parameter space more uniformly than a random grid.
- Racing eliminates clearly under-performing candidates early, cutting compute by ~60–70% vs full grid search.
- Primary selection metric: **RMSE** (minimise); R² reported alongside.

```{r grids}
# We need to know the number of predictors to set mtry range after prep
prep_recipe <- prep(base_recipe, training = train_raw)
n_preds     <- ncol(bake(prep_recipe, new_data = NULL, all_predictors()))
cat("Number of predictors after preprocessing:", n_preds, "\n")

# XGBoost grid — mtry uses integer range (column counts)
xgb_grid <- grid_space_filling(
  trees(          range = c(300L,   2000L)),
  tree_depth(     range = c(3L,     10L)),
  min_n(          range = c(5L,     40L)),
  loss_reduction( range = c(-5,     2)),      # log10 scale
...
**Strategy:** Select the candidate with the lowest mean CV RMSE
(i.e., the single best model, not "best within 1 SE" — we prioritise
predictive accuracy over parsimony since both models have identical
complexity structure and the goal is minimising RMSE).
...
## 13 · Finalise & Fit on Full Training Data
