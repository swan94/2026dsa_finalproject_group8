...
**Choice:** 5-fold cross-validation repeated 2 times, stratified on `yield_mg_ha` (cut into quartiles for stratification).
...
**Search strategy:** Latin Hypercube Space-Filling Design (size = 40 per model) followed by **Racing via ANOVA** (`tune_race_anova`).
...
**Strategy:** Select the candidate with the lowest mean CV RMSE (i.e., the single best model, not "best within 1 SE" — we prioritise predictive accuracy over parsimony since both models have identical complexity structure and the goal is minimising RMSE).
