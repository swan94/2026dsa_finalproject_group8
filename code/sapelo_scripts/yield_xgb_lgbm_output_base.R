...
# Helper: save a ggplot with a consistent size
...
# ── Save data glimpse to a text file ────────────────────────
sink(file.path(out_dir, "data_glimpse.txt"))
cat("=== Training Data ===\n"); glimpse(train_raw)
cat("\n=== Test Data ===\n");    glimpse(test_raw)
sink()
message("Text saved: data_glimpse.txt")
...
if (nrow(missing_df) > 0) {
  p_missing <- missing_df %>%
    ...
    labs(title = "Missing Data — Training Set", x = NULL, y = "% Missing") +
    theme_bw()
...
# Visualize XGBoost grid
p_xgb_grid <- ggplot(data = xgb_grid,
...
# Save the chosen final hyperparameters for each model
...
p_split_dist <- ggplot() +
  ...
  labs(title = "Yield Distribution — Train vs Validation Split",
...
# ── Combined train vs validation metrics summary ─────────────
...
cat("\n=== Metrics Summary (Train vs Validation) ===\n")
...
p_rmse_bar <- metrics_summary %>%
  ...
  labs(title = "RMSE — Train vs Validation",
...
p_r2_bar <- metrics_summary %>%
  ...
  geom_text(aes(label = round(rsq, 4)),
            position = position_dodge(0.6), vjust = -0.4, size = 3.5) +
  scale_fill_manual(values = c("Training" = "#2c7bb6", "Validation" = "#d7191c")) +
  labs(title = "R² — Train vs Validation",
...
p_xgb_resid <- xgb_preds_val %>%
  ...
  labs(title = "XGBoost — Residuals vs Fitted (Validation)",
...
p_lgbm_resid <- lgbm_preds_val %>%
  ...
  geom_point(alpha = 0.3, color = "#1a9641", size = 0.8) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  labs(title = "LightGBM — Residuals vs Fitted (Validation)",
       x = "Fitted (Predicted) Yield", y = "Residual") +
  theme_bw()
...
# Re-fit on yield_train for VIP (same data used for metrics)
...
#  A single human-readable text file with every key number.

sink(file.path(out_dir, "00_run_summary.txt"))
...
cat("\n============================================================\n")
sink()
message("Summary saved: 00_run_summary.txt")
