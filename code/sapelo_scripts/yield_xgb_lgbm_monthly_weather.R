...
#  overwrite the base-model run happening simultaneously.
#
#  model_tag  → prefix on every filename  (e.g. mw_01_...)
#  out_dir    → dedicated subfolder

model_tag <- "mw"     # mw = monthly weather
out_dir   <- "/home/hm64666/final_project/outputs/monthly_weather"
...
# ── Step 2: Load Data ────────────────────────────────────────
train_path <- "/home/hm64666/final_project/data/training/training_model_growing_season_weather_final.csv"
test_path  <- "/home/hm64666/final_project/data/testing/testing_model_monthly_weather_final.csv"
...
cat("=== Training Data (Monthly Weather) ===\n"); glimpse(train_raw)
cat("\n=== Test Data (Monthly Weather) ===\n");    glimpse(test_raw)
sink()
...
p_yield_dist <- ggplot(train_raw, aes(x = yield_mg_ha)) +
  ...
  labs(title = "Monthly Weather — Distribution of Corn Yield (Training Set)",
...
if (nrow(missing_df) > 0) {
  p_missing <- missing_df %>%
    ...
    labs(title = "Monthly Weather — Missing Data (Training Set)", x = NULL, y = "% Missing") +
    theme_bw()
...
write_lines(
  paste("Monthly Weather — Predictors after preprocessing:", n_preds),
...
p_xgb_grid <- ggplot(data = xgb_grid, aes(x = tree_depth, y = min_n)) +
  ...
  labs(title = "Monthly Weather — XGBoost Hyperparameter Grid (Latin Hypercube)",
...
p_split_dist <- ggplot() +
  ...
  labs(title = "Monthly Weather — Yield Distribution: Train vs Validation Split",
...
cat("\n=== Monthly Weather — Validation Metrics ===\n")
...
cat("\n=== Monthly Weather — Metrics Summary (Train vs Validation) ===\n")
...
p_rmse_bar <- metrics_summary %>%
  ...
  labs(title = "Monthly Weather — RMSE: Train vs Validation",
...
p_r2_bar <- metrics_summary %>%
  ...
  geom_text(aes(label = round(rsq, 4)),
            position = position_dodge(0.6), vjust = -0.4, size = 3.5) +
  scale_fill_manual(values = c("Training" = "#2c7bb6", "Validation" = "#d7191c")) +
  labs(title = "Monthly Weather — R²: Train vs Validation",
...
p_xgb_pred <- xgb_preds_val %>%
  ...
  labs(title = "Monthly Weather — XGBoost: Predicted vs Observed (Validation)",
...
p_lgbm_pred <- lgbm_preds_val %>%
  ...
  geom_abline() + geom_smooth(method = "lm") +
  labs(title = "Monthly Weather — LightGBM: Predicted vs Observed (Validation)",
       x = "Observed Yield (Mg/ha)", y = "Predicted Yield (Mg/ha)") +
  theme_bw()
...
p_xgb_resid <- xgb_preds_val %>%
  ...
  labs(title = "Monthly Weather — XGBoost: Residuals vs Fitted (Validation)",
...
p_lgbm_resid <- lgbm_preds_val %>%
  ...
  geom_point(alpha = 0.3, color = "#1a9641", size = 0.8) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  labs(title = "Monthly Weather — LightGBM: Residuals vs Fitted (Validation)",
       x = "Fitted (Predicted) Yield", y = "Residual") +
  theme_bw()
...
p_xgb_vip <- xgb_vi %>%
  ...
  labs(title = "Monthly Weather — XGBoost: Variable Importance (Top 20)", y = NULL) +
  theme_bw()
...
p_lgbm_vip <- lgbm_vi %>%
  ...
  scale_x_continuous(expand = c(0, 0)) +
  labs(title = "Monthly Weather — LightGBM: Variable Importance (Top 20)", y = NULL) +
  theme_bw()
...
cat("\n✓ All Monthly Weather outputs written to:", out_dir, "\n")
