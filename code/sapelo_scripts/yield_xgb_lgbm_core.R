...
# BUG FIX 1: was "yw" (yearly weather tag) — changed to "cm" for core model
model_tag <- "cm"     # cm = core model
out_dir   <- "/home/hm64666/final_project/outputs/core_model"
...
# ── Step 2: Load Data ────────────────────────────────────────
train_path <- "/home/hm64666/final_project/data/training/training_model_core_final.csv"
test_path  <- "/home/hm64666/final_project/data/testing/testing_model_core_final.csv"

train_raw <- read_csv(train_path)
test_raw  <- read_csv(test_path)

# Save data glimpse
# BUG FIX 2: labels said "Yearly Weather" — corrected to "Core Model"
sink(file.path(out_dir, paste0(model_tag, "_data_glimpse.txt")))
cat("=== Training Data (Core Model) ===\n"); glimpse(train_raw)
...
# BUG FIX 3: all plot titles corrected from "Yearly Weather" to "Core Model"
p_yield_dist <- ggplot(train_raw, aes(x = yield_mg_ha)) +
  geom_histogram(bins = 60, fill = "#2c7bb6", color = "white", alpha = 0.85) +
  labs(title = "Core Model — Distribution of Corn Yield (Training Set)",
...
if (nrow(missing_df) > 0) {
  p_missing <- missing_df %>%
    ...
    labs(title = "Core Model — Missing Data (Training Set)", x = NULL, y = "% Missing") +
    theme_bw()
...
# BUG FIX 4: added step_rm() for date columns (date_planted, date_harvested).
# XGBoost/LightGBM require an all-numeric matrix — Date columns cause a crash
# at tune_race_anova() time (confirmed from monthly model error log).
yield_recipe <- recipe(yield_mg_ha ~ ., data = train_raw) %>%
  step_rm(any_of(c("date_planted", "date_harvested"))) %>%  # drop date columns
...
write_lines(
  paste("Core Model — Predictors after preprocessing:", n_preds),
...
p_xgb_grid <- ggplot(data = xgb_grid, aes(x = tree_depth, y = min_n)) +
  ...
  labs(title = "Core Model — XGBoost Hyperparameter Grid (Latin Hypercube)",
...
p_split_dist <- ggplot() +
  ...
  labs(title = "Core Model — Yield Distribution: Train vs Validation Split",
...
cat("\n=== Core Model — Metrics Summary (Train vs Validation) ===\n")
...
p_rmse_bar <- metrics_summary %>%
  ...
  labs(title = "Core Model — RMSE: Train vs Validation",
...
p_r2_bar <- metrics_summary %>%
  ...
  geom_text(aes(label = round(rsq, 4)),
            position = position_dodge(0.6), vjust = -0.4, size = 3.5) +
  scale_fill_manual(values = c("Training" = "#2c7bb6", "Validation" = "#d7191c")) +
  labs(title = "Core Model — R²: Train vs Validation",
...
p_xgb_pred <- xgb_preds_val %>%
  ...
  labs(title = "Core Model — XGBoost: Predicted vs Observed (Validation)",
...
p_lgbm_pred <- lgbm_preds_val %>%
  ...
  geom_abline() + geom_smooth(method = "lm") +
  labs(title = "Core Model — LightGBM: Predicted vs Observed (Validation)",
       x = "Observed Yield (Mg/ha)", y = "Predicted Yield (Mg/ha)") +
  theme_bw()
...
p_xgb_resid <- xgb_preds_val %>%
  ...
  labs(title = "Core Model — XGBoost: Residuals vs Fitted (Validation)",
...
p_lgbm_resid <- lgbm_preds_val %>%
  ...
  geom_point(alpha = 0.3, color = "#1a9641", size = 0.8) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  labs(title = "Core Model — LightGBM: Residuals vs Fitted (Validation)",
       x = "Fitted (Predicted) Yield", y = "Residual") +
  theme_bw()
...
p_xgb_vip <- xgb_vi %>%
  ...
  labs(title = "Core Model — XGBoost: Variable Importance (Top 20)", y = NULL) +
  theme_bw()
...
p_lgbm_vip <- lgbm_vi %>%
  ...
  scale_x_continuous(expand = c(0, 0)) +
  labs(title = "Core Model — LightGBM: Variable Importance (Top 20)", y = NULL) +
  theme_bw()
...
cat("\n✓ All Core Model outputs written to:", out_dir, "\n")
