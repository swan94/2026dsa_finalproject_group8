...
cat("=== Training Data (Yearly Weather) ===\n"); glimpse(train_raw)
cat("\n=== Test Data (Yearly Weather) ===\n");    glimpse(test_raw)
sink()
...
p_yield_dist <- ggplot(train_raw, aes(x = yield_mg_ha)) +
  ...
  labs(title = "Yearly Weather — Distribution of Corn Yield (Training Set)",
...
if (nrow(missing_df) > 0) {
  p_missing <- missing_df %>%
    ...
    labs(title = "Yearly Weather — Missing Data (Training Set)", x = NULL, y = "% Missing") +
    theme_bw()
...
write_lines(
  paste("Yearly Weather — Predictors after preprocessing:", n_preds),
...
p_xgb_grid <- ggplot(data = xgb_grid, aes(x = tree_depth, y = min_n)) +
  ...
  labs(title = "Yearly Weather — XGBoost Hyperparameter Grid (Latin Hypercube)",
...
p_split_dist <- ggplot() +
  ...
  labs(title = "Yearly Weather — Yield Distribution: Train vs Validation Split",
...
cat("\n=== Yearly Weather — Validation Metrics ===\n")
...
cat("\n=== Yearly Weather — Metrics Summary (Train vs Validation) ===\n")
...
p_rmse_bar <- metrics_summary %>%
  ...
  labs(title = "Yearly Weather — RMSE: Train vs Validation",
...
p_r2_bar <- metrics_summary %>%
  ...
  geom_text(aes(label = round(rsq, 4)),
            position = position_dodge(0.6), vjust = -0.4, size = 3.5) +
  scale_fill_manual(values = c("Training" = "#2c7bb6", "Validation" = "#d7191c")) +
  labs(title = "Yearly Weather — R²: Train vs Validation",
...
p_xgb_pred <- xgb_preds_val %>%
  ...
  labs(title = "Yearly Weather — XGBoost: Predicted vs Observed (Validation)",
...
p_lgbm_pred <- lgbm_preds_val %>%
  ...
  geom_abline() + geom_smooth(method = "lm") +
  labs(title = "Yearly Weather — LightGBM: Predicted vs Observed (Validation)",
       x = "Observed Yield (Mg/ha)", y = "Predicted Yield (Mg/ha)") +
  theme_bw()
...
p_xgb_resid <- xgb_preds_val %>%
  ...
  labs(title = "Yearly Weather — XGBoost: Residuals vs Fitted (Validation)",
...
p_lgbm_resid <- lgbm_preds_val %>%
  ...
  geom_point(alpha = 0.3, color = "#1a9641", size = 0.8) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  labs(title = "Yearly Weather — LightGBM: Residuals vs Fitted (Validation)",
       x = "Fitted (Predicted) Yield", y = "Residual") +
  theme_bw()
...
p_xgb_vip <- xgb_vi %>%
  ...
  labs(title = "Yearly Weather — XGBoost: Variable Importance (Top 20)", y = NULL) +
  theme_bw()
...
p_lgbm_vip <- lgbm_vi %>%
  ...
  scale_x_continuous(expand = c(0, 0)) +
  labs(title = "Yearly Weather — LightGBM: Variable Importance (Top 20)", y = NULL) +
  theme_bw()
...
cat("\n✓ All Yearly Weather outputs written to:", out_dir, "\n")
